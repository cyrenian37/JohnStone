...
{
    public class HSOrder : ViewModelBase
    {
        ...
        private DataSource.vEmpDept _displaySeller;
        private DataSource.vEmpDept _displayDeliver;

        private ObservableCollection<HSOrderItem> _orderItems;
        ...
        private string GetInvoiceNo()
        {
            ...
            try
            {
                var l = lDataContext.getUniqInvoiceNo().Single();
                return l.InvoiceNo;
            }
            catch
            {
                MessageBox.Show("Failed to get the InvoiceNo.");
                return String.Empty;
            }
            ...
            //return CommonFunction.GetCompanyInfo().Initial + DateTime.Now.ToString("yyMMdd") + (InvoiceInt + 1).ToString("D3");
        ...
        private Boolean SavingEnded()
        {
            ...
            if (lInvoice == null)
            {
                ...
                if (this.Invoice.InvoiceNo == string.Empty)
                {
                    MessageBox.Show("Cannot get the Invoice No.");
                    return false;
                }
            ...
            foreach (HSOrderItem l in this.OrderItems)
            {
        ...
        private void OnDeleteCommandExecuted(object id)
        {
                       ...
                       MessageBoxButton.OKCancel) == MessageBoxResult.OK)
            {
                ...
                this.OrderItems = new ObservableCollection<HSOrderItem>();
        ...
        void ProductWindow_selecteProductCommand(List<JSProduct> selectedProduct)
        {
            ...
            foreach (JSProduct l in selectedProduct)
            {
                ...
                if (SelectedCustomer == null)
                    OrderItems.Add(new HSOrderItem { Id = -1, ProductId = l.Id, Quantity = 0, UnitPrice = 0, VatId = 1, SellingUnitId = 1 });
                else
                {
                    ...
                    lDefaultPrice = lDataContext.getHistoryUnitPrice(SelectedCustomer.Id, l.Id, l.SelectedUnitId, this.Invoice.InvoiceDate);

                    OrderItems.Add(new HSOrderItem
                    {
                        Id = -1,
                        ProductId = l.Id,
        ...
        public DataSource.Customer SelectedCustomer
        {
            ...
            set
            {
                ...
                {
                    foreach (HSOrderItem l in OrderItems)
                    {
        ...
        public ObservableCollection<HSOrderItem> OrderItems
        {
            get
            {
                if (_orderItems == null)
            ...
            set
            {
                ...
                this.OnPropertyChanged(() => this.OrderItems);
            }
        }
